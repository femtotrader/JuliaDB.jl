...

{
    ...
    "title": "Overview",
    "category": "section",
    "text": "JuliaDB is a package for working with large persistent data sets. We recognized the need for a convenient, end-to-end tool to load common multi-dimensional datasets quickly, perform filter, aggregation, sort and join operations on them and save the results efficiently for later. We also wanted the tool to readily make use of Julia's built-in parallelism to acheive close to the best performance on any machine or cluster. This is the motivation behind JuliaDB.JuliaDB is Julia all the way down. This means queries can be composed with Julia code that may use its vast ecosystem of packages, incurring no overhead as computation reaches the data.JuliaDB is based on Dagger and IndexedTables, providing a array-like data model where the indices of the array are themselves data. Over time, we hope to expand this to include dense arrays and other Julia array types like AxisArrays. JuliaDB also provides all the familiar relational database operations that are optimized to use the indexed nature of the data model.Given a set of CSV files, JuliaDB builds and saves an index that allows the data to be accessed efficiently in the future. The \"ingest\" operation converts data to an efficient memory-mappable binary format. We plan to extend JuliaDB to use data from various other sources."
},

{
    ...
    "title": "Installation",
    "category": "section",
    "text": "JuliaDB works on Julia 0.6 or higher. To install it, run:Pkg.clone(\"https://github.com/JuliaComputing/JuliaDB.jl.git\")"
},
...

{
    ...
    "category": "section",
    "text": "To use JuliaDB you may start Julia with a few worker processes, for example, julia -p 4. Let's load some sample CSV files that are in JuliaDB's test folder:using JuliaDB\n\npath = Pkg.dir(\"JuliaDB\", \"test\", \"sample\")\n\nsampledata = loadfiles(path, indexcols=[\"date\", \"ticker\"])loadfiles loads all files under a given directory. If you started julia with many processes (e.g. with julia -p 4, or by doing addprocs(N) before using JuliaDB) then loadfiles will enlist all available processes to read the csv files in parallel. If you wanted to specify a subset of files to load or files in different directories, you can pass the file names as a vector as in place of the directory path. JuliaDB exports the glob function from Glob.jl to help you with this.Using the indexcols option, here we specified that loadfiles should use date and ticker columns as the index for the data. The index columns will be used to sort the data for efficient queries. See the API reference for loadfiles for all available options.Notice that the output says DTable with 288 rows in 6 chunks. loadfiles creates a distributed table (DTable) with as many chunks as the input files. The loaded chunks are distributed across available worker processes. loadfiles will also save metadata about the contents of the files in a directory named .juliadb in the directory with the files (or in the current working directory if a vector of filenames is passed). This means, the next time the files are loaded, it will not need to actually parse them to know what's in them. However a file will be parsed once an operation requires the data in it.Another way to load data into JuliaDB is using ingest. ingest reads and saves the data in an efficient memory-mappable binary storage format for faster re-reading. You can also add new files to an existing dataset using ingest!.To set some context, our sample data contains monthly aggregates of stock prices (open, high, low, close values) as well as volume traded for 4 stocks (GOOGL, GS, KO, XRX) in the years 2010 to 2015. Each file contains a single year's data."
},

{
    ...
    "category": "section",
    "text": "You can save a DTable to disk at any point:save(t, \"<outputdir>\")This will create <outputdir> and save the individual chunks of the data separately.A saved dataset can be loaded with load:data = load(\"<outpudir>\")"
},
...

{
    ...
    "title": "Indexing",
    "category": "section",
    "text": "Most lookup and filtering operations on DTable can be done via indexing. Our sampledata object behaves like a 2-d array, accepting two indices, each values or range of values from the index columns.You can get a specific value by indexing it by the exact index:sampledata[Date(\"2010-06\"), \"GOOGL\"] # Get GOOGL's data for June 2010note: Note\nNote that Date(\"2010-06\") is automatically converted to Date(\"2010-06-01\"). Since we don't have a Date type which can only represent month and a year (it might be an extraneous abstraction anyway), we have used the first of each month to represent the month itself.Above, we are indexing the table with a specific index value (2010-06-01, \"GOOGL\"). Here our DTable behaved like a dictionary, giving the value stored at a given key. The result is a NamedTuple object containing 5 fields which of the same names as the data columns.One can also get a subset of the DTable by indexing with a range or a sorted vector of index values:sampledata[:, [\"GOOG\", \"KO\"]]Fetches all values in the data for the stock symbols GOOG and KO.sampledata[Date(\"2012-01\"):Dates.Month(1):Date(\"2014-12\"), [\"GOOG\", \"KO\"]]Fetches all values in the data for the stock symbols GOOG and KO in the years 2012 - 2014Range indexing always returns a DTable so that you can apply any other JuliaDB operation on the result of indexing.Minutiae: notice the range we have used in the last example: Date(\"2012-01\"):Dates.Month(1):Date(\"2014-12\"). This says \"from 2012-01-01 to 2014-12-01 in steps of 1 month\". Date/DateTime ranges in Julia need to be specified with an increment such as Dates.Month(1). If your dataset contains timestamps in the millisecond resolution, for example, you'd need to specify Dates.Millisecond(1) as the increment, and so on."
},
...

{
    ...
    "category": "section",
    "text": "A location in the coordinate space of an array often has multiple possible descriptions. This is especially common when describing data at different levels of detail. For example, a point in time can be expressed at the level of seconds, minutes, or hours. In our test dataset, we might want to look at quarterly values.This can be accomplished using the convertdim function. It accepts a DTable, a dimension number to convert, a function or dictionary to apply to indices in that dimension, and an aggregation function (the aggregation function is needed in case the mapping is many-to-one). You can optionally give a new name to the converted dimension using the name keyword argument.The following call therefore gives the quarterly aggregates for our data:convertdim(sampledata, 1, Dates.firstdayofquarter,\n                     agg=agg_ohlcv, name=:quarter)The mental model here is, first every value in dimension 1 is converted using the function Dates.firstdayofquarter, i.e. to the first day of the quarter that date falls in. Next, the values in the table which correspond to the same indices (e.g. all values for the GOOG stock in 1st quarter of 2010) are aggregated together using agg."
},
